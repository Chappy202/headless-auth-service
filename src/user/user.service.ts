...
export class UserService {
  ...
  async updateProfile(id: number, updateData: UpdateProfileDto) {
    ...
    if (updateData.password) {
      updateValues.password = await bcrypt.hash(updateData.password, process.env.SALT_ROUNDS || 10);
    }
    if (updateData.mfaEnabled !== undefined) updateValues.mfaEnabled = updateData.mfaEnabled;
